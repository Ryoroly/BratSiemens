//la fel ca v7 daor ca pot sa dau comenzi din serial monitor
...
void setup() {
  ...
  Serial.println("1. BLE: S xul yul - Seteaza dimensiunile zonei");
  Serial.println("2. BLE: T x y id - Transporta obiectul");
  Serial.println("3. SERIAL: x y id - Transporta obiectul direct");
  Serial.println("   (exemplu: 100 200 1)");
  Serial.println("   ID obiecte: 1=cube, 2=cylinder, 3=halfcircle");
  Serial.println("                4=arch, 5=triangle, 6=rectangle");
  Serial.println("=====================================");
  Serial.println("Astept conexiune BLE sau comenzi din Serial...");
  Serial.println("=====================================");
}

void loop() {
  // Verificam comenzi din Serial Monitor (chiar dacă nu e conectat BLE)
  checkSerialCommands();
  ...
  if (central) {
    ...
    while (central.connected()) {
      // Verificam comenzi din Serial Monitor si in timpul conexiunii BLE
      checkSerialCommands();
...
// Functie pentru verificarea comenzilor din Serial Monitor
void checkSerialCommands() {
  if (Serial.available() && !robotBusy) {
    ...
    if (serialCommand.length() > 0) {
      ...
      // Verificam daca e comanda directa (doar x y id) sau comanda standard
      if (serialCommand.startsWith("S") || serialCommand.startsWith("T")) {
        // Comanda standard - procesam direct
        processCommand(serialCommand);
      } else {
        // Comanda directa - parsam x y id si convertim la T x y id
        handleDirectSerialCommand(serialCommand);
      }
    }
  }
}

// Functie pentru comenzi directe din Serial (format: x y id)
void handleDirectSerialCommand(String command) {
  Serial.println(">>> PROCESEZ COMANDA DIRECTA SERIAL...");
  ...
  float x = command.substring(0, firstSpace).toFloat();
  float y = command.substring(firstSpace + 1, secondSpace).toFloat();
  int id = command.substring(secondSpace + 1).toInt();
  
  // Verificăm dacă valorile sunt valide
  if (x == 0 && command.substring(0, firstSpace) != "0") {
    Serial.println(">>> EROARE: Valoare X invalida!");
    return;
  }
  
  if (y == 0 && command.substring(firstSpace + 1, secondSpace) != "0") {
    Serial.println(">>> EROARE: Valoare Y invalida!");
    return;
  }
  
  if (id < 1 || id > 6) {
    Serial.println(">>> EROARE: ID obiect invalid! Folositi 1-6");
    return;
  }
  
  Serial.print(">>> COMANDA SERIAL PARSATA: X=");
  Serial.print(x);
  Serial.print(", Y=");
  Serial.print(y);
  Serial.print(", ID=");
  ...
  // Convertim la format T si procesam
  String convertedCommand = "T " + String(x) + " " + String(y) + " " + String(id);
  processCommand(convertedCommand);
}
