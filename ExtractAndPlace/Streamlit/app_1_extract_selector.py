...
image_paths = sorted([
    os.path.join(input_folder, f) for f in os.listdir(input_folder) if f.lower().endswith(valid_ext)
])
if not image_paths:
    st.info("No images found.")
    st.stop()

# Initialize or read session index
...
def save_and_advance(img_path, lh, ls, lv, uh, us, uv, output_folder, selected_set):
    # timestamped folder per source set
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_folder = os.path.join(output_folder, f"{selected_set}_{ts}")
    os.makedirs(run_folder, exist_ok=True)
    # regenerate mask and contours
    ...
    count = 0
    for idx_cnt, cnt in enumerate(cnts):
        x1,y1,w1,h1 = cv2.boundingRect(cnt)
        obj_bgr = img_local[y1:y1+h1, x1:x1+w1]
        mask_crop = m[y1:y1+h1, x1:x1+w1]
        # convert to BGRA so imwrite saves correct colors
        obj_rgba = cv2.cvtColor(obj_bgr, cv2.COLOR_BGR2BGRA)
        obj_rgba[:,:,3] = mask_crop
        out_path = os.path.join(run_folder, f"{os.path.splitext(os.path.basename(img_path))[0]}_obj_{idx_cnt}.png")
...
st.button(
    ...
    args=(img_path, lh, ls, lv, uh, us, uv, OUTPUT_FOLDER, selected_set),
)
