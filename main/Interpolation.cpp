...
ServoPosition calculateInterpolatedPosition(float x, float y) {
  ...
  result.m2_shoulder = constrain(base_m2 + M2_OFFSET_CORNERS, 15, 165);//se aplica offset la miscarea pentru m2
  ...
  result.m3_elbow = constrain(base_m3 + M3_OFFSET_CORNERS, 0, 180);//se aplica offset la miscarea pentru m3
